...
function App() {
  ...
  return (
    // 1. Wrap entire App in AuthProvider to provide User Context
    <AuthProvider>
      <HashRouter>
        <Toaster
        ...
        <Routes>
          ...
          {/* ==========================================
              PROTECTED: RECEPTION MODULE
              Allowed: 'receptionist', 'admin' (optional)
              ========================================== */}
          <Route element={<ProtectedRoute allowedRoles={['receptionist', 'admin']} />}>
            <Route path="/reception" element={<ReceptionLayout />}>
              ...
              <Route path="register-appointment" element={<RegisterPatientView />} />
          ...
          <Route element={<ProtectedRoute allowedRoles={['pharmacist']} />}>
            <Route path="/pharmacist" element={<PharmacistLayout />}>
              ...
              <Route
                path="sales-report"
                element={<PlaceholderComponent title="Sales Reports" />}
              />
              <Route
                path="feedback"
                element={<PlaceholderComponent title="Feedback System" />}
              />
              <Route path="profile" element={<ProfileView />} />
  ...
  <div className="flex flex-col items-center justify-center h-full">
    <div className="p-8 bg-zinc-100 dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800">
      ...
      <p className="text-zinc-500 text-xs font-bold mt-2">
        The requested path does not exist in the{" "}
        <span className="text-indigo-500">{node}</span> cluster.
      </p>
    </div>
  </div>
);
...
const GlobalNotFound = () => (
  <div className="h-screen w-full flex items-center justify-center bg-zinc-950 text-white">
    <div className="text-center">
      ...
      <p className="text-xs font-black uppercase tracking-[0.5em] -mt-10 relative z-10">
        System Protocol Failure
      </p>
      <button
        onClick={() => (window.location.href = "#/login")}
        className="mt-8 px-6 py-2 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-full"
      >
        Reset to Login
      </button>
    </div>
