...
const UserActivities = () => {
  ...
  const activities = [
    {
      id: 1,
      user: { name: "Nurse Jesse", email: "j.j@hospital.com", initials: "NJ" },
      type: "login",
      description: "Nurse Jesse logged into the system from Station 04.",
      time: "10 Jan 2026, 08:32 AM",
      ip: "192.168.1.45",
    },
    {
      id: 2,
      user: { name: "Dr. Sarah", email: "s.s@hospital.com", initials: "SS" },
      type: "updated",
      description: "Changed health record for Patient #PT-1024.",
      time: "10 Jan 2026, 08:15 AM",
      ip: "192.168.1.12",
    },
    {
      id: 3,
      user: {
        name: "Admin Michael",
        email: "m.m@hospital.com",
        initials: "AM",
      },
      type: "created",
      description: "Added a new staff member account: Pharmacist John.",
      time: "09 Jan 2026, 04:50 PM",
      ip: "192.168.1.2",
    },
    {
      id: 4,
      user: { name: "Nurse Jesse", email: "j.j@hospital.com", initials: "NJ" },
      type: "deleted",
      description: "Removed a draft lab test request #LAB-88.",
      time: "09 Jan 2026, 02:22 PM",
      ip: "192.168.1.45",
    },
  ];
  ...
  const getTypeStyle = (type) => {
    ...
    return styles[type] || styles.logout;
  };
        ...
        <div className="px-6 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-2 font-medium">
              Keep track of who did what and when in the hospital system.
            </p>
          </div>
        ...
        <div className="px-6 py-3 border-t border-b border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row gap-4 items-center justify-between mb-4">
          <div className="relative flex-1 min-w-[240px] group">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-4 bg-zinc-50 dark:bg-zinc-800/50 border border-transparent rounded-xl text-sm font-medium focus:bg-white dark:focus:bg-zinc-800 focus:border-primary-500/30 transition-all outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-2 px-3 h-10 bg-zinc-50 dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700">
            ...
            <input
              ...
              className="bg-transparent text-xs font-black uppercase tracking-tight outline-none"
            />
          </div>
          ...
          <select
            ...
            className="h-10 px-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-black uppercase tracking-widest outline-none focus:border-primary-500 text-zinc-500"
          >
            <option value="">All Actions</option>
            <option value="login">Logged In</option>
            <option value="created">Added New</option>
            <option value="updated">Changed/Edited</option>
      ...
      <div className="flex-1 m-4 card-base shadow-xl border-opacity-50 overflow-hidden flex flex-col mb-4">
            ...
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800">
              {activities.map((activity) => (
                <tr
                  ...
                  data-context-type="activity"
                  data-context-name={`Log #${activity.id} - ${activity.user.name}`}
                  data-context-id={activity.id}
                >
                  ...
                  <td className="px-8 py-5 whitespace-nowrap">
                    <span
                      className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${getTypeStyle(
                        activity.type
                      )}`}
                    >
                      {activity.type === "login"
        ...
        <div className="px-8 py-5 border-t border-zinc-50 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/50 flex justify-between items-center shrink-0">
          <span className="text-xs font-bold text-zinc-500 uppercase tracking-tight">
            Showing 4 of 48 items
          </span>
          <div className="flex gap-2">
            <button className="px-5 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 border border-zinc-200 dark:border-zinc-800 rounded-xl opacity-50">
              Prev
            </button>
            <button className="px-5 py-2 text-[10px] font-black uppercase tracking-widest text-white bg-primary-600 rounded-xl shadow-lg shadow-primary-500/20">
              1
            </button>
            <button className="px-5 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800">
              2
            </button>
            <button className="px-5 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800">
              Next
...
const DetailModal = ({ activity, onClose, getTypeStyle }) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
    <div
      className="absolute inset-0 bg-zinc-950/60 backdrop-blur-sm animate-in fade-in duration-300"
      onClick={onClose}
    />
    <div className="relative w-full max-w-lg card-base overflow-hidden animate-in zoom-in-95 duration-200 shadow-2xl">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <span
                className={`inline-block px-3 py-1 rounded-lg text-[10px] font-black border tracking-widest ${getTypeStyle(
                  activity.type
                )}`}
              >
